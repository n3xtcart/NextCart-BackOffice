...
const prodotti = [
  ...
  function toggleQuantityInput() {
    ...
    document.getElementById('nomeInput').style.display = filtro === 'nome' ? 'flex' : 'none';
    document.getElementById('quantitaInput').style.display = filtro === 'quantita' ? 'flex' : 'none';
    document.getElementById('categoriaInput').style.display = filtro === 'categoria' ? 'flex' : 'none';
    filterProducts();
  }
  ...
  function filterProducts() {
    ...
    if (filtro === 'nome') {
      ...
      prodottiFiltrati = prodotti.filter(p => p.name.toLowerCase().includes(nome));
    } else if (filtro === 'quantita') {
      const quantita = parseInt(document.getElementById('quantita').value);
      if (!isNaN(quantita)) {
        prodottiFiltrati = prodotti.filter(p => p.quantity === quantita);
      }
    } else if (filtro === 'categoria') {
      const categoria = document.getElementById('categoriaSelect').value;
      if (categoria !== "") {
        prodottiFiltrati = prodotti.filter(p => p.category === categoria);
      }
    }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Event listeners per filtro
    document.getElementById('filtro').addEventListener('change', toggleQuantityInput);
